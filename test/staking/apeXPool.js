...
describe("apexPool UT", function() {
  ...
  describe("stake", function() {
    it("reverted when stake invalid amount", async function() {
      await expect(apeXPool.stake(0, lockDuration)).to.be.revertedWith("ap.stake: INVALID_AMOUNT");
    });

    it("reverted when invalid lock interval", async function() {
      await expect(apeXPool.stake(10000, invalidLockDuration)).to.be.revertedWith("ap._stake: INVALID_LOCK_INTERVAL");
    });
